...
def gams_fproc(q,j,t,s):
    return GAMS_FPROC_OVERRIDES.get((q,j,t,s), 1.0)
...
def gams_cvh(l,h,j,t):
    ...
    return GAMS_CACH[(l,t)] + 1000.0*GAMS_DPCDH[(h,j)]
...
def gams_cvc(m,j,k,t):
    return GAMS_CAC[(m,t)] + 1200.0*GAMS_DPCD[(j,k)]

def gams_cvccd(o,k,n,t):
    return GAMS_CACCD[(o,t)] + 1400.0*GAMS_DCDC[(k,n)]

def gams_cvpp(z,j,jp,t):
    return GAMS_CACPP[(z,t)] + 1400.0*GAMS_DCPP[(j,jp)]
...
def build_model(data: OptimizerData):
    ...
    def final_regular_cap_rule(m, i, j, t, s):
        q = _final_q(i)
        cap = GAMS_PACK_CAP[(i,j)] * GAMS_ENV[(i,t,s)] * gams_fproc(q,j,t,s)
    ...
    m.sub_cap = pyo.Constraint(
        m.I, m.J, m.T, m.S,
        rule=lambda m,i,j,t,s: m.sub[i,j,t,s] <= GAMS_MAXSUB[(i,j)] * m.YP[j]
    )
    ...
    m.buffer_cap = pyo.Constraint(
        m.I, m.J, m.T, m.S,
        rule=lambda m,i,j,t,s: m.buffer[i,j,t,s] <= GAMS_BUFFER_CAP[(i,j)]
    )

    # Capacidades XR/XE de procesos intermedios: CAPXRAjus y CAPXE=5%.
    def proc_reg_cap_rule(m,q,j,t,s):
        cap = GAMS_PROCESS_BASE_CAP[q][j] * gams_fproc(q,j,t,s)
    ...
    def prod_cost(m, s):
        ...
        subcontract = sum(
            GAMS_SUB_COST[(i,j)]*m.sub[i,j,t,s]
            for i in m.I for j in m.J for t in m.T
        )
        proc_regular = sum(
    ...
    def inv_cost(m, s):
        ...
        plant = sum(
            GAMS_BUFFER_COST[(i,j)]*m.buffer[i,j,t,s]
            for i in m.I for j in m.J for t in m.T
        )
