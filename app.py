...
def reset_state():
    ...
    st.session_state.design_evaluations = []
...
if "design_evaluations" not in st.session_state:
    st.session_state.design_evaluations = []
...
with st.sidebar:
    ...
    page = st.radio("NAVEGACIÓN", ["Dashboard", "Predicciones y tendencias", "Diseño y optimización", "Transporte y logística", "Escenarios", "Laboratorio DT", "Comparación", "Historial de ciclos", "Lecciones", "Glossary"], index=0)
    st.divider()
...
# ---------- Parametrización independiente del próximo ciclo ----------
with st.expander("⚙️ Parametrizar próximo ciclo", expanded=False):
    st.caption("Estos valores se usarán como entradas del próximo ciclo A1→A2→A3→A4 y quedarán guardados en su historial.")
    cp1, cp2 = st.columns(2)
    pnext = st.session_state.physical
    with cp1:
        pnext["supply"] = st.slider("Suministro del próximo ciclo", 0.05, 1.00, float(pnext["supply"]), 0.01, key="cycle_supply")
        pnext["demand"] = st.slider("Demanda del próximo ciclo", 0.50, 2.00, float(pnext["demand"]), 0.01, key="cycle_demand")
        pnext["transport"] = st.slider("Transporte del próximo ciclo", 0.05, 1.00, float(pnext["transport"]), 0.01, key="cycle_transport")
        pnext["processing"] = st.slider("Procesamiento del próximo ciclo", 0.05, 1.00, float(pnext["processing"]), 0.01, key="cycle_processing")
    with cp2:
        pnext["quality"] = st.slider("Calidad del próximo ciclo", 0.05, 1.00, float(pnext["quality"]), 0.01, key="cycle_quality")
        pnext["lead"] = st.slider("Lead time del próximo ciclo", 0.50, 2.50, float(pnext["lead"]), 0.01, key="cycle_lead")
        pnext["intensity"] = st.slider("Intensidad de disrupción del próximo ciclo", 0.00, 1.00, float(pnext["intensity"]), 0.01, key="cycle_intensity")
        pnext["persistence"] = st.slider("Persistencia de disrupción del próximo ciclo", 0.00, 1.00, float(pnext["persistence"]), 0.01, key="cycle_persistence")
    st.session_state.physical = pnext
...
def render_document_aligned_analytics(pkg):
    """Visualizaciones adicionales alineadas con la lógica APDT del documento."""
    if pkg is None:
        return

    history = st.session_state.get("history", [])
    r1, r2, r3, r4 = pkg["a1"], pkg["a2"], pkg["a3"], pkg["a4"]

    st.markdown("### 🧠 Memoria adaptativa y regulación SCIS")
    if history:
        rows = []
        layer_map = {"L0":0, "L1":1, "L2":2, "L3":3, "L4":4}
        for item in history:
            a1, a2, a4 = item["a1"], item["a2"], item["a4"]
            layer = str(a4.final_decision.get("scis_layer","L0"))
            rows.append({
                "Ciclo": int(item["cycle"]),
                "Memoria A2": float(a2.memory_after),
                "Presión": float(a1.current_pressure),
                "SCIS": float(layer_map.get(layer, 0)),
            })
        h = pd.DataFrame(rows).set_index("Ciclo")
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("#### Evolución de memoria A2")
            st.line_chart(h[["Memoria A2"]], height=270)
        with c2:
            st.markdown("#### Presión y nivel SCIS")
            sc = h[["Presión","SCIS"]].copy()
            # Normalización visual del nivel L0-L4 a 0-1 para compararlo con presión.
            sc["SCIS normalizado"] = sc["SCIS"] / 4.0
            st.line_chart(sc[["Presión","SCIS normalizado"]], height=270)
            st.caption("SCIS normalizado: L0=0%, L1=25%, L2=50%, L3=75%, L4=100%.")

    st.markdown("### 🌡️ Mapa experimental de presión y respuesta")
    st.caption(
        "Mapa didáctico construido con A1 mediante simulaciones What-if. "
        "No ejecuta URSP–SCIS repetidamente y no modifica el historial."
    )
    base = pkg["input"].copy()
    intensity_levels = [0.0, 0.25, 0.50, 0.75, 1.0]
    persistence_levels = [0.0, 0.25, 0.50, 0.75, 1.0]
    heat_rows = []
    for pers in persistence_levels:
        row = {"Persistencia": pers}
        for inten in intensity_levels:
            trial = base.copy()
            trial["intensity"] = inten
            trial["persistence"] = pers
            try:
                sim = execute_cycle(
                    trial, st.session_state.cycle,
                    st.session_state.previous_pressure,
                    st.session_state.memory_level,
                    copy.deepcopy(st.session_state.memory_repository_a2),
                    copy.deepcopy(st.session_state.memory_repository_a4),
                    "Mapa experimental"
                )
                row[f"I={inten:.2f}"] = float(sim["a4"].outcome.get("viability_after",0))
            except Exception:
                row[f"I={inten:.2f}"] = None
        heat_rows.append(row)
    heat = pd.DataFrame(heat_rows).set_index("Persistencia")
    heat_display = heat.copy()
    for col in heat_display.columns:
        heat_display[col] = heat_display[col].map(
    ...
    st.caption("Filas = persistencia de la disrupción; columnas = intensidad; celdas = viabilidad posterior estimada por el ciclo APDT.")

    st.markdown("### 💰 Viabilidad, utilidad y riesgo del diseño URSP–SCIS")
    result = st.session_state.get("optimizer_result")
    if result:
        v = float(result.get("expected_viability",0))
        profit = float(result.get("expected_profit",0))
        cvar = float(result.get("cvar",0))
        shortage = float(result.get("expected_shortage",0))
        q1,q2,q3,q4 = st.columns(4)
        q1.metric("Viabilidad", f"{v:.1%}")
        q2.metric("Utilidad esperada", f"{profit:,.0f}")
        q3.metric("CVaR", f"{cvar:,.0f}")
        q4.metric("Faltante esperado", f"{shortage:,.0f}")
        st.caption("Estos indicadores provienen de la última corrida real del optimizador, no de la simulación del mapa.")
    else:
        st.info("Ejecuta Diseño y optimización para incorporar utilidad, CVaR y faltantes al análisis.")


def render_cycle_conclusion(pkg):
    """Resumen narrativo trazable de A1→A4 + LEAN + optimizador."""
    if pkg is None:
        return
    r1, r2, r3, r4 = pkg["a1"], pkg["a2"], pkg["a3"], pkg["a4"]
    vals, avg, diag = lean_pack(pkg)
    # lean_pack devuelve el diagnóstico en una estructura que puede variar
    # según la versión. Lo normalizamos sin modificar los algoritmos.
    if isinstance(diag, dict):
        dominant_waste = diag.get("dominant_waste", "No determinado")
        dominant_value = float(diag.get("dominant_value", 0.0))
    else:
        dominant_waste = str(diag) if diag is not None else "No determinado"
        dominant_value = float(max(vals.values())) if isinstance(vals, dict) and vals else 0.0
    outcome = r4.outcome
    fd = r4.final_decision
    result = st.session_state.get("optimizer_result")

    st.markdown("### 🧾 Conclusión integrada del ciclo")
    pred_delta = float(r1.predicted_pressure-r1.current_pressure)
    if pred_delta > 0.03:
        pred_text = "anticipa un aumento de la presión"
    elif pred_delta < -0.03:
        pred_text = "anticipa una reducción de la presión"
    else:
        pred_text = "anticipa una presión relativamente estable"

    lines = [
        f"🔮 **A1:** {pred_text}: {r1.current_pressure:.1%} → {r1.predicted_pressure:.1%}, con confianza {r1.prediction_confidence:.1%}.",
        f"🧠 **A2:** la memoria adaptativa cambia de {r2.memory_before:.1%} a {r2.memory_after:.1%}.",
        f"🌐 **A3:** el futuro con mayor probabilidad es **{r3.most_probable_scenario}**.",
        f"⚙️ **A4:** la decisión adaptativa utiliza **{fd.get('scis_layer','L0')}** y alcanza éxito estimado de {outcome.get('decision_success',0):.1%}.",
        f"♻️ **LEAN:** el desperdicio dominante es **{dominant_waste}** ({dominant_value:.1%}); desperdicio global {avg:.1%}.",
        f"📊 **Resultado del ciclo:** servicio {outcome.get('service_after',0):.1%} y viabilidad {outcome.get('viability_after',0):.1%}.",
    ]
    if result:
        lines.append(
            f"🏭 **URSP–SCIS:** última configuración optimizada con viabilidad esperada "
            f"{float(result.get('expected_viability',0)):.1%}, utilidad esperada "
            f"{float(result.get('expected_profit',0)):,.0f} y faltante esperado "
            f"{float(result.get('expected_shortage',0)):,.0f}."
        )
    st.markdown(
        '<div class="decision-summary">' + "<br><br>".join(lines) + "</div>",
        unsafe_allow_html=True
    )

def render_predictive_center(pkg):
    """Centro predictivo: visualiza únicamente salidas ya calculadas por A1, A3 y A4."""
    st.markdown("### 🔮 Predicciones y tendencias del Digital Twin")
    st.caption("Lectura integrada del estado actual, la predicción A1, los futuros A3 y la respuesta observada por ciclo.")

    if pkg is None:
        st.info("Ejecuta al menos un ciclo para generar predicciones y escenarios.")
        return

    r1, r3, r4 = pkg["a1"], pkg["a3"], pkg["a4"]
    o = r4.outcome

    p1, p2, p3, p4 = st.columns(4)
    p1.metric("Presión actual", f"{float(r1.current_pressure):.1%}")
    p2.metric("Presión predicha", f"{float(r1.predicted_pressure):.1%}",
              delta=f"{float(r1.predicted_pressure-r1.current_pressure):+.1%}")
    p3.metric("Confianza A1", f"{float(r1.prediction_confidence):.1%}")
    p4.metric("Viabilidad después", f"{float(o.get('viability_after',0)):.1%}")

    st.markdown("#### 📈 Presión operacional: observada vs. predicha")
    pressure_df = pd.DataFrame(
        {"Valor": [float(r1.current_pressure), float(r1.predicted_pressure)]},
        index=["Actual", "Predicha"]
    )
    st.bar_chart(pressure_df, height=260)

    st.markdown("#### 🔮 Distribución de futuros generados por A3")
    scenario_rows = []
    for s in r3.scenarios:
        scenario_rows.append({
            "Escenario": str(s.get("scenario_name", "Escenario")),
            "Probabilidad": float(s.get("probability", 0.0))
        })
    if scenario_rows:
        pred_sdf = pd.DataFrame(scenario_rows).set_index("Escenario")
        st.bar_chart(pred_sdf, height=280)

    # Trayectorias/impactos A3: se grafican solo campos que el algoritmo realmente entrega.
    impact_rows = []
    for s in r3.scenarios:
        row = {"Escenario": str(s.get("scenario_name", "Escenario"))}
        for label, keys in {
            "Presión": ("scenario_pressure", "pressure", "projected_pressure"),
            "Suministro": ("supply_availability", "supply"),
            "Demanda": ("demand_level", "demand"),
            "Transporte": ("transport_availability", "transport"),
            "Procesamiento": ("processing_capacity", "capacity"),
            "Lead time": ("lead_time_index", "lead_time"),
        }.items():
            for key in keys:
                if key in s and isinstance(s.get(key), (int, float)):
                    row[label] = float(s[key])
                    break
        if len(row) > 1:
            impact_rows.append(row)
    if impact_rows:
        impact_df = pd.DataFrame(impact_rows).set_index("Escenario")
        st.markdown("#### 📊 Condiciones proyectadas por escenario A3")
        st.line_chart(impact_df, height=320)
        st.caption("Cada línea corresponde a una salida numérica generada por A3; no es una extrapolación añadida por la interfaz.")

    # Comparación entre estado actual y siguiente estado decidido por A4.
    nxt = next_physical_from_package(pkg)
    cur = pkg["input"]
    state_df = pd.DataFrame({
        "Actual": [float(cur["supply"]), float(cur["demand"]), float(cur["transport"]), float(cur["processing"]), float(cur["quality"]), float(cur["lead"])],
        "Siguiente estado A4": [float(nxt["supply"]), float(nxt["demand"]), float(nxt["transport"]), float(nxt["processing"]), float(nxt["quality"]), float(nxt["lead"])],
    }, index=["Suministro", "Demanda", "Transporte", "Procesamiento", "Calidad", "Lead time"])
    st.markdown("#### 🧭 Estado actual vs. siguiente estado proyectado por A4")
    st.bar_chart(state_df, height=320)

    history = st.session_state.history
    if history:
        st.markdown("#### 📉 Evolución temporal del Digital Twin")
        hist_rows = []
        for item in history:
            a1 = item["a1"]
            a4 = item["a4"]
            hist_rows.append({
                "Ciclo": int(item["cycle"]),
                "Presión actual": float(a1.current_pressure),
                "Presión predicha": float(a1.predicted_pressure),
                "Viabilidad": float(a4.outcome.get("viability_after",0)),
                "Servicio": float(a4.outcome.get("service_after",0)),
            })
        hdf = pd.DataFrame(hist_rows).set_index("Ciclo")
        st.line_chart(hdf[["Presión actual","Presión predicha"]], height=280)
        st.markdown("#### 📊 Viabilidad, servicio y umbral por ciclo")
        threshold = 0.85
        try:
            if st.session_state.get("optimizer_result"):
                # El umbral visible del optimizador se mantiene como referencia cuando existe una corrida.
                threshold = float(st.session_state.get("last_viability_threshold", threshold))
        except Exception:
            pass
        hdf["Umbral de viabilidad"] = threshold
        st.line_chart(hdf[["Viabilidad","Servicio","Umbral de viabilidad"]], height=300)
        st.caption(f"Referencia visual del umbral de viabilidad: {threshold:.0%}.")

        last_row = hdf.iloc[-1]
        if float(last_row["Presión predicha"]) > float(last_row["Presión actual"]) + 0.03:
            st.warning("🟠 A1 anticipa un aumento relevante de presión operacional para el siguiente estado.")
        elif float(last_row["Presión predicha"]) < float(last_row["Presión actual"]) - 0.03:
            st.success("🟢 A1 anticipa una reducción de la presión operacional.")
        else:
            st.info("🟡 A1 anticipa una presión relativamente estable.")

    st.markdown(
        '<div class="explain"><b>Cómo leer estas gráficas:</b> A1 anticipa la presión del siguiente estado; '
        'A3 distribuye la incertidumbre entre futuros posibles; A4 muestra el efecto de la decisión sobre '
        'viabilidad y servicio. Las gráficas no reemplazan los algoritmos: visualizan sus salidas.</div>',
        unsafe_allow_html=True
    )


def register_design_evaluation(result, od, plant_choices, cd_choices):
    """Guarda una sola fila por configuración para comparar corridas reales del solver."""
    row = {
        "Plantas": " + ".join(plant_choices) if plant_choices else "Ninguna",
        "CD": " + ".join(cd_choices) if cd_choices else "Ninguno",
        "Nº plantas": len(plant_choices),
        "Nº CD": len(cd_choices),
        "Viabilidad": float(result.get("expected_viability", 0.0)),
        "Utilidad esperada": float(result.get("expected_profit", 0.0)),
        "CVaR": float(result.get("cvar", 0.0)),
        "Faltante esperado": float(result.get("expected_shortage", 0.0)),
        "Umbral": float(od.theta),
        "Cumple umbral": float(result.get("expected_viability",0.0)) >= float(od.theta),
    }
    key = (row["Plantas"], row["CD"])
    previous = st.session_state.design_evaluations
    st.session_state.design_evaluations = [
        x for x in previous if (x["Plantas"], x["CD"]) != key
    ] + [row]


def render_design_comparison():
    """Compara solo configuraciones que el usuario ya resolvió con URSP–SCIS."""
    evaluations = st.session_state.get("design_evaluations", [])
    st.markdown("#### 🏭 Viabilidad de plantas y centros evaluados")
    st.caption(
        "La comparación se construye con corridas reales del solver. "
        "No asigna puntajes inventados a una instalación individual."
    )
    if not evaluations:
        st.info("Selecciona una combinación de plantas/CD y ejecuta el solver. Cada configuración resuelta quedará guardada aquí para compararla.")
        return

    df = pd.DataFrame(evaluations)
    show = df.copy()
    show["Cumple umbral"] = show["Cumple umbral"].map({True:"Sí", False:"No"})
    st.dataframe(
        show.style.format({
            "Viabilidad":"{:.1%}",
            "Utilidad esperada":"{:,.0f}",
            "CVaR":"{:,.0f}",
            "Faltante esperado":"{:,.0f}",
            "Umbral":"{:.1%}",
        }),
        use_container_width=True,
        hide_index=True
    ...
    # Mapa de calor con resultados REALES de las configuraciones ya resueltas.
    st.markdown("#### 🌡️ Mapa de calor — Plantas × Centros de Distribución")
    st.caption(
        "Cada celda corresponde a una configuración que ya fue ejecutada con URSP–SCIS. "
        "El color representa la viabilidad esperada; las combinaciones aún no evaluadas quedan vacías."
    )

    heat_source = (
        df.groupby(["Nº plantas", "Nº CD"], as_index=False)["Viabilidad"]
          .mean()
          .sort_values(["Nº plantas", "Nº CD"])
    )
    heat_source["Viabilidad_pct"] = heat_source["Viabilidad"] * 100.0
    heat_source["Etiqueta"] = heat_source["Viabilidad"].map(lambda x: f"{x:.1%}")
        ...
        "layer": [
            {
                ...
                "encoding": {
                    "x": {"field": "Nº CD", "type": "ordinal", "title": "Número de centros de distribución"},
                    "y": {"field": "Nº plantas", "type": "ordinal", "title": "Número de plantas", "sort": "descending"},
                    "color": {
                        "field": "Viabilidad_pct", "type": "quantitative", "title": "Viabilidad (%)",
                        "scale": {"scheme": "viridis", "domainMin": 0, "domainMax": 100}
                    },
                    "tooltip": [
                        {"field": "Nº plantas", "type": "ordinal", "title": "Plantas"},
                        {"field": "Nº CD", "type": "ordinal", "title": "CD"},
                        {"field": "Viabilidad_pct", "type": "quantitative", "title": "Viabilidad (%)", "format": ".1f"}
                    ]
                }
            },
            {
                ...
                "encoding": {
                    "x": {"field": "Nº CD", "type": "ordinal"},
                    "y": {"field": "Nº plantas", "type": "ordinal", "sort": "descending"},
                    "text": {"field": "Etiqueta"},
                    "color": {
                        "condition": {"test": "datum.Viabilidad_pct < 55", "value": "black"},
                        "value": "white"
                    }
                }
            }
    ...
    st.vega_lite_chart(heat_source, heat_spec, use_container_width=True)

    st.info(
        f"📌 El mapa contiene {len(heat_source)} configuración(es) evaluada(s). "
        "Para llenar nuevas celdas, cambia el número de plantas/CD y vuelve a ejecutar el optimizador."
    )

    chart = df.copy()
    chart["Configuración"] = chart["Plantas"] + " | " + chart["CD"]
    st.bar_chart(chart.set_index("Configuración")[["Viabilidad"]], height=300)

    best = df.loc[df["Viabilidad"].idxmax()]
    st.success(
        f"🟢 Entre las configuraciones evaluadas hasta ahora, la mayor viabilidad es "
        f"{best['Viabilidad']:.1%}: plantas **{best['Plantas']}** y CD **{best['CD']}**."
    )
    st.caption(
        "Esta conclusión aplica únicamente a las configuraciones que ya fueron ejecutadas con los mismos parámetros. "
        "Para comparar otra combinación, selecciónala y vuelve a ejecutar URSP–SCIS."
    )

    if st.button("🗑️ Limpiar comparación de diseños", key="clear_design_evaluations"):
        st.session_state.design_evaluations = []
        st.rerun()
...
if page == "Diseño y optimización":
    ...
    st.caption("Configura la red y los parámetros operativos que alimentan realmente el MILP URSP–SCIS.")

    # Restauración reproducible del caso base GAMS.
    if st.button("↩️ Restaurar parámetros originales GAMS", key="restore_gams_optimizer"):
        st.session_state.optimizer_data = OptimizerData.from_gams_defaults()
        st.session_state.optimizer_base_demand = dict(st.session_state.optimizer_data.demand)
        for _key in list(st.session_state.keys()):
            if str(_key).startswith(("param_", "prob_")) or _key in {"number_of_plants", "number_of_cds"}:
                del st.session_state[_key]
        st.rerun()

    net1, net2 = st.columns(2)
    with net1:
        od.number_of_plants = int(st.number_input(
            "Número de plantas / fábricas", min_value=1,
            value=max(1, int(getattr(od, "number_of_plants", 3))), step=1,
            key="number_of_plants"
        ))
    with net2:
        od.number_of_cds = int(st.number_input(
            "Número de centros de distribución", min_value=1,
            value=max(1, int(getattr(od, "number_of_cds", 4))), step=1,
            key="number_of_cds"
        ))

    dynamic_plants = [f"j{i}" for i in range(1, od.number_of_plants + 1)]
    dynamic_cds = [f"k{i}" for i in range(1, od.number_of_cds + 1)]
    st.info(
        f"🌐 Red parametrizada: **{od.number_of_plants} plantas** y **{od.number_of_cds} CD**. "
        f"El MILP se construirá con {len(dynamic_plants)} nodos de planta y {len(dynamic_cds)} nodos CD."
    )

    st.markdown("##### 📦 Demanda por producto")
    d1, d2, d3 = st.columns(3)
    demand_cols = {"mantequilla": d1, "yogurt": d2, "leche_past": d3}
    demand_labels = {"mantequilla": "Mantequilla", "yogurt": "Yogurt", "leche_past": "Leche pasteurizada"}
    for prod, col in demand_cols.items():
        with col:
            od.demand[prod] = st.number_input(
                f"Demanda · {demand_labels[prod]}", min_value=0.0,
                value=float(od.demand.get(prod, st.session_state.optimizer_base_demand.get(prod, 0.0))),
                step=500.0, key=f"param_demand_{prod}"
            )

    st.markdown("##### 🏭 Capacidades y criterios de decisión")
    c1, c2, c3 = st.columns(3)
    with c1:
        od.capacity_per_plant = st.number_input(
            "Capacidad por planta", min_value=1000.0, max_value=500000.0,
            value=float(od.capacity_per_plant), step=1000.0, key="param_capacity_plant"
        )
        od.capacity_per_cd = st.number_input(
            "Capacidad por CD", min_value=1000.0, max_value=500000.0,
            value=float(od.capacity_per_cd), step=1000.0, key="param_capacity_cd"
        )
    with c2:
        od.eps_y = st.slider("Tolerancia homeostática (eps_y)", 0.01, 0.30, float(od.eps_y), 0.01, key="param_eps_y")
        od.theta = st.slider("Umbral de viabilidad (theta)", 0.40, 0.95, float(od.theta), 0.01, key="param_theta")
    with c3:
        od.rhoR = st.slider("Aversión al riesgo (rhoR)", 0.0, 5.0, float(od.rhoR), 0.1, key="param_rhoR")
        od.alpha = st.slider("Confianza CVaR (alpha)", 0.50, 0.99, float(od.alpha), 0.01, key="param_alpha")

    with st.expander("🌩️ Condiciones operativas por escenario", expanded=False):
        st.caption("Estos factores entran directamente al MILP. 1,00 representa el nivel de referencia; valores menores reducen disponibilidad/capacidad.")
        for s in ["s1", "s2", "s3", "s4", "s5"]:
            st.markdown(f"**{s}**")
            o1, o2, o3, o4 = st.columns(4)
            od.supply_factor[s] = o1.number_input("Suministro", 0.10, 1.50, float(od.supply_factor[s]), 0.05, key=f"param_supply_{s}")
            od.transport_factor[s] = o2.number_input("Transporte", 0.10, 1.50, float(od.transport_factor[s]), 0.05, key=f"param_transport_{s}")
            od.processing_factor[s] = o3.number_input("Procesamiento", 0.10, 1.50, float(od.processing_factor[s]), 0.05, key=f"param_processing_{s}")
            od.disruption[s] = o4.number_input("Disrupción", 0.00, 1.00, float(od.disruption[s]), 0.05, key=f"param_disruption_{s}")

    with st.expander("⚙️ Parámetros avanzados del MILP", expanded=False):
        a1, a2, a3 = st.columns(3)
        with a1:
            od.share_max = st.slider("Participación máxima por planta (share_max)", 0.10, 1.00, float(od.share_max), 0.05, key="param_share_max")
        with a2:
            od.profit_min = st.number_input("Beneficio de referencia para riesgo", min_value=0.0, value=float(od.profit_min), step=10000000.0, format="%.0f", key="param_profit_min")
        with a3:
            od.penalty_short_factor = st.number_input("Factor de penalización por faltante", min_value=0.0, max_value=10.0, value=float(od.penalty_short_factor), step=0.05, key="param_penalty_short")

        st.markdown("**Costos unitarios por producto**")
        for prod in ["mantequilla", "yogurt", "leche_past"]:
            x1, x2, x3 = st.columns(3)
            od.production_unit_cost[prod] = x1.number_input(f"Producción · {demand_labels[prod]}", min_value=0.0, value=float(od.production_unit_cost[prod]), step=50.0, key=f"param_prod_cost_{prod}")
            od.transport_unit_cost[prod] = x2.number_input(f"Transporte · {demand_labels[prod]}", min_value=0.0, value=float(od.transport_unit_cost[prod]), step=10.0, key=f"param_trans_cost_{prod}")
            od.inventory_unit_cost[prod] = x3.number_input(f"Inventario · {demand_labels[prod]}", min_value=0.0, value=float(od.inventory_unit_cost[prod]), step=5.0, key=f"param_inv_cost_{prod}")

        st.markdown("**Costos fijos de instalaciones**")
        fp_cols = st.columns(2)
        for idx, j in enumerate(dynamic_plants):
            base_val = float(od.fixed_plants.get(j, od.fixed_plants.get("j1", 12000000.0)))
            od.fixed_plants[j] = fp_cols[idx % 2].number_input(f"Costo fijo {j}", min_value=0.0, value=base_val, step=100000.0, format="%.0f", key=f"param_fixed_{j}")
        fc_cols = st.columns(2)
        for idx, k in enumerate(dynamic_cds):
            base_val = float(od.fixed_cds.get(k, od.fixed_cds.get("k1", 5000000.0)))
            od.fixed_cds[k] = fc_cols[idx % 2].number_input(f"Costo fijo {k}", min_value=0.0, value=base_val, step=100000.0, format="%.0f", key=f"param_fixed_{k}")

        st.caption("Los coeficientes internos SCIS (Big-M, lambdas, etas y memoria) permanecen protegidos para conservar la formulación matemática del modelo.")

    st.markdown("#### 2. Definir diseño")
    design_mode = st.radio(
        "Modo de diseño de la red",
        ["Optimización libre", "Diseño fijado"],
        horizontal=True,
        key="network_design_mode",
        help="En optimización libre, HiGHS decide qué instalaciones activar. En diseño fijado, tú eliges cuáles quedan abiertas."
    )

    if design_mode == "Optimización libre":
        plant_choices = dynamic_plants.copy()
        cd_choices = dynamic_cds.copy()
        od.plant_design_status = None
        od.cd_design_status = None
        st.success(
            "🧠 Diseño libre: el optimizador decidirá cuáles plantas y centros de distribución activar dentro de la red parametrizada."
        )
    else:
        d1,d2 = st.columns(2)
        with d1:
            plant_choices = st.multiselect(
                "Plantas que deben quedar activas",
                dynamic_plants,
                default=dynamic_plants,
                key="fixed_dynamic_plants"
            )
        with d2:
            cd_choices = st.multiselect(
                "Centros de distribución que deben quedar activos",
                dynamic_cds,
                default=dynamic_cds,
                key="fixed_dynamic_cds"
            )
        od.plant_design_status = {j: (1 if j in plant_choices else 0) for j in dynamic_plants}
        od.cd_design_status = {k: (1 if k in cd_choices else 0) for k in dynamic_cds}
        st.caption(
            "🔒 Diseño fijado: la selección obliga matemáticamente las binarias YP/YCD a 1 o 0."
        )
    ...
    if st.button("🚀 EJECUTAR OPTIMIZACIÓN URSP–SCIS", type="primary", use_container_width=True):
        with st.spinner("Resolviendo el MILP URSP–SCIS..."):
            try:
                ...
                st.session_state.last_viability_threshold = float(od.theta)

                # Para la comparación se registra el diseño realmente resuelto.
                # En modo libre no confundimos nodos disponibles con nodos activados.
                _solved_design = st.session_state.optimizer_result.get("design", {})
                _solved_plants = _solved_design.get("plants", {})
                _solved_cds = _solved_design.get("cds", {})
                if od.plant_design_status is None:
                    evaluated_plants = [
                        j for j in dynamic_plants
                        if int(round(float(_solved_plants.get(j, 0)))) == 1
                    ]
                else:
                    evaluated_plants = plant_choices
                if od.cd_design_status is None:
                    evaluated_cds = [
                        k for k in dynamic_cds
                        if int(round(float(_solved_cds.get(k, 0)))) == 1
                    ]
                else:
                    evaluated_cds = cd_choices

                register_design_evaluation(
                    st.session_state.optimizer_result, od, evaluated_plants, evaluated_cds
                )
            except Exception as exc:
    ...
    if result:
        ...
        # Verificación visible de la red que realmente llegó al optimizador.
        solved_design = result.get("design", {})
        solved_plants = solved_design.get("plants", {})
        solved_cds = solved_design.get("cds", {})
        active_plants = [
            j for j in dynamic_plants
            if int(round(float(solved_plants.get(j, 0)))) == 1
        ]
        active_cds = [
            k for k in dynamic_cds
            if int(round(float(solved_cds.get(k, 0)))) == 1
        ]
        # Compatibilidad con versiones del solver que solo devuelven los conteos.
        if not solved_plants and design_mode == "Diseño fijado":
            active_plants = list(plant_choices)
        if not solved_cds and design_mode == "Diseño fijado":
            active_cds = list(cd_choices)

        st.markdown("#### 🌐 Verificación de la red optimizada")
        vr1, vr2, vr3, vr4 = st.columns(4)
        vr1.metric("Plantas evaluadas", str(len(dynamic_plants)))
        vr2.metric("CD evaluados", str(len(dynamic_cds)))
        vr3.metric("Plantas activadas", str(len(active_plants) if solved_plants or design_mode == "Diseño fijado" else result.get("plants_active", "—")))
        vr4.metric("CD activados", str(len(active_cds) if solved_cds or design_mode == "Diseño fijado" else result.get("cds_active", "—")))

        st.markdown(
            f'<div class="explain"><b>Red evaluada:</b> {len(dynamic_plants)} plantas × {len(dynamic_cds)} CD<br>'
            f'<b>Plantas disponibles:</b> {", ".join(dynamic_plants)}<br>'
            f'<b>CD disponibles:</b> {", ".join(dynamic_cds)}<br>'
            f'<b>Plantas activadas por HiGHS:</b> {", ".join(active_plants) if active_plants else "No expuestas individualmente por esta corrida"}<br>'
            f'<b>CD activados por HiGHS:</b> {", ".join(active_cds) if active_cds else "No expuestos individualmente por esta corrida"}<br>'
            f'<b>Modo:</b> {design_mode}<br>'
            f'<b>Viabilidad obtenida:</b> {float(result.get("expected_viability",0)):.1%}</div>',
            unsafe_allow_html=True
        )

        # Guarda metadatos de la última red; se reutilizarán en el mapa de calor.
        st.session_state.last_network_verification = {
            "number_of_plants": len(dynamic_plants),
            "number_of_cds": len(dynamic_cds),
            "available_plants": list(dynamic_plants),
            "available_cds": list(dynamic_cds),
            "active_plants": list(active_plants),
            "active_cds": list(active_cds),
            "design_mode": design_mode,
            "viability": float(result.get("expected_viability", 0.0)),
            "expected_profit": float(result.get("expected_profit", 0.0)),
            "cvar": float(result.get("cvar", 0.0)),
            "expected_shortage": float(result.get("expected_shortage", 0.0)),
        }
        ...
        # Interpretación automática: compara el gap REAL alcanzado con la tolerancia
        # que realmente se configuró para HiGHS. No se usa un umbral fijo distinto.
        if achieved_gap is not None:
            achieved_gap = float(achieved_gap)
            configured_gap = float(mip_gap) if mip_gap is not None else 0.02
            eps = 1e-9

            if achieved_gap <= eps:
                st.success(
                    "✅ Convergencia: HiGHS alcanzó un gap prácticamente nulo; "
                    "la solución está óptimamente certificada dentro de la precisión reportada."
                )
            elif achieved_gap <= configured_gap + eps:
                st.success(
                    f"✅ Convergencia aceptada: el MIP gap alcanzado ({achieved_gap:.2%}) "
                    f"cumple la tolerancia configurada ({configured_gap:.2%})."
                )
            else:
                st.warning(
                    f"🟠 Solución factible con convergencia pendiente: el MIP gap alcanzado "
                    f"({achieved_gap:.2%}) supera la tolerancia configurada ({configured_gap:.2%}). "
                    "Puede aumentarse el tiempo de solución o continuar mejorando la formulación MILP."
                )
        else:
            st.caption(
                "MIP gap alcanzado: no disponible en la información devuelta por HiGHS/Pyomo; "
                "la aplicación no asigna ni supone un valor."
            )
        ...
        # Conclusión automática trazable de la corrida URSP–SCIS.
        st.markdown("### 🧾 Conclusión automática de la corrida")
        expected_v = float(result.get("expected_viability", 0.0))
        expected_profit = float(result.get("expected_profit", 0.0))
        cvar_value = float(result.get("cvar", 0.0))
        expected_shortage = float(result.get("expected_shortage", 0.0))
        configured_gap = float(mip_gap) if mip_gap is not None else None
        achieved_gap_value = float(achieved_gap) if achieved_gap is not None else None

        if optimal:
            convergence_text = "HiGHS reportó terminación óptima."
        elif achieved_gap_value is not None and configured_gap is not None and achieved_gap_value <= configured_gap + 1e-9:
            convergence_text = (
                f"La solución es factible y el gap alcanzado ({achieved_gap_value:.2%}) "
                f"cumple la tolerancia configurada ({configured_gap:.2%})."
            )
        elif achieved_gap_value is not None and configured_gap is not None:
            convergence_text = (
                f"La solución es factible, pero el gap alcanzado ({achieved_gap_value:.2%}) "
                f"supera la tolerancia configurada ({configured_gap:.2%}); la convergencia sigue pendiente."
            )
        else:
            convergence_text = "El gap alcanzado no está disponible; no se supone un nivel de convergencia."

        if expected_v >= float(od.theta):
            viability_text = (
                f"La viabilidad esperada ({expected_v:.1%}) cumple el umbral configurado "
                f"({float(od.theta):.1%})."
            )
        else:
            viability_text = (
                f"La viabilidad esperada ({expected_v:.1%}) está por debajo del umbral configurado "
                f"({float(od.theta):.1%})."
            )

        conclusion_lines = [
            f"🏭 **Red:** se evaluaron {len(dynamic_plants)} plantas y {len(dynamic_cds)} CD; "
            f"el modelo activó {result.get('plants_active', len(active_plants))} plantas y "
            f"{result.get('cds_active', len(active_cds))} CD.",
            f"📈 **Viabilidad:** {viability_text}",
            f"💰 **Resultado económico:** utilidad esperada {expected_profit:,.0f} y CVaR {cvar_value:,.0f}.",
            f"📦 **Servicio:** faltante esperado {expected_shortage:,.0f}; el escenario con menor nivel de servicio es "
            f"**{critical_name}**, con servicio {critical_service:.1%} y faltante {critical_shortage:,.0f}.",
            f"🎯 **Solver:** {convergence_text}",
        ]
        st.markdown(
            '<div class="decision-summary">' + "<br><br>".join(conclusion_lines) + "</div>",
            unsafe_allow_html=True,
        )
        st.caption(
            "Conclusión generada exclusivamente con los resultados de la última corrida del optimizador; "
            "no reemplaza ni modifica las decisiones del MILP."
        )
        ...
        if solved_plants or solved_cds:
            st.markdown("##### 🏭 Verificación del diseño de red")
            vc1, vc2 = st.columns(2)
            with vc1:
                st.write("**Plantas (YP)**")
                for j in dynamic_plants:
                    requested = int((od.plant_design_status or {}).get(j, solved_plants.get(j,0)))
            ...
            with vc2:
                ...
                for k in dynamic_cds:
                    requested = int((od.cd_design_status or {}).get(k, solved_cds.get(k,0)))
                    obtained = int(round(float(solved_cds.get(k,0))))
                    st.write(f"{k}: solicitado {requested} → solución {obtained}")

            if od.plant_design_status is not None and od.cd_design_status is not None:
                design_ok = (
                ...
                active_p = [j for j in dynamic_plants if int(round(float(solved_plants.get(j,0)))) == 1]
                active_k = [k for k in dynamic_cds if int(round(float(solved_cds.get(k,0)))) == 1]
                st.success(
                    f"🧠 Diseño optimizado libremente: {len(active_p)}/{len(dynamic_plants)} plantas y "
                    f"{len(active_k)}/{len(dynamic_cds)} CD fueron activados por HiGHS."
                )
    ...
    # Evita que esta vista caiga en el else final de navegación (Glossary).
    st.stop()


if page == "Predicciones y tendencias":
    render_predictive_center(package)
    render_document_aligned_analytics(package)
    render_cycle_conclusion(package)
    st.stop()
...
if page == "Dashboard":
    ...
    if not st.session_state.history:
        ...
        st.markdown("#### ⚙️ Parámetros utilizados en cada ciclo")
        input_rows=[]
        for item in st.session_state.history:
            inp=item.get("input", {})
            input_rows.append({
                "Ciclo": item.get("cycle"),
                "Escenario": item.get("scenario", "—"),
                "Suministro": inp.get("supply"),
                "Demanda": inp.get("demand"),
                "Transporte": inp.get("transport"),
                "Procesamiento": inp.get("processing"),
                "Calidad": inp.get("quality"),
                "Lead time": inp.get("lead"),
                "Intensidad": inp.get("intensity"),
                "Persistencia": inp.get("persistence"),
            })
        df_inputs=pd.DataFrame(input_rows).set_index("Ciclo")
        st.dataframe(df_inputs, use_container_width=True)
        st.download_button(
            "⬇️ Exportar parámetros de ciclos CSV",
            df_inputs.to_csv().encode("utf-8-sig"),
            "digital_twin_parametros_ciclos.csv",
            "text/csv",
            use_container_width=True,
        )
