...
st.set_page_config(
    page_title="Supply Chain Digital Twin Learning Lab",
    page_icon="🌐",
    layout="wide"
)
...
with st.sidebar:
    ...
    page = st.radio("NAVEGACIÓN", ["Dashboard", "Escenarios", "Laboratorio DT", "Comparación", "Historial de ciclos", "Lecciones", "Glossary"], index=0)
    st.divider()
    ...
    if last is None:
        ...
        sidebar_kpis = [
            ...
            ("Viabilidad Worst-case",f"{rr4.outcome.get('viability_worst_case',rr4.outcome.get('viability_after',0)):.2f}"),
            ("Nivel de servicio",f"{rr4.outcome.get('service_after',0):.2f}"),
            ("Utilidad esperada",f"{rr2.expected_net_utility:.2f}"),
            ("CVaR (proxy didáctico)",f"{rr3.worst_case_probability:.2f}"),
            ("Presión operacional",f"{rr1.current_pressure:.2f}"),
            ("Confianza de decisión",f"{rr2.decision_confidence:.2f}"),
...
b1,b2,b3,b4 = st.columns([1.15,1.15,0.95,2.05])
with b1:
...
with b3:
    reset_scenario = st.button("↩ Reiniciar escenario", use_container_width=True)
with b4:
...
if reset_scenario:
    d = SCENARIO_DEFAULTS[st.session_state.scenario_name].copy()
    st.session_state.physical = d
    st.session_state.cycle = 1
    st.session_state.previous_pressure = 0.18
    st.session_state.memory_level = 0.35
...
def render_a3(r3):
    ...
    st.bar_chart(sdf.set_index("Escenario"), height=180)
...
def render_lean(pkg):
    ...
    st.bar_chart(ldf.set_index("Desperdicio"),height=220)
    cols=st.columns(3)
...
if page == "Dashboard":
    ...
    st.markdown(
        '<div class="explain"><b>Objetivo del laboratorio:</b> modifica las condiciones de la cadena, '
        'ejecuta ciclos sucesivos y observa cómo la predicción, la memoria, los escenarios, la decisión, '
        'el desperdicio LEAN y el nuevo estado cambian de forma trazable.</div>',
        unsafe_allow_html=True
    )
    if package is None:
    ...
    st.markdown(
        '<div class="explain"><b>Regla del experimento:</b> cada cambio en una variable recorre nuevamente '
        '<b>A1 → A2 → A3 → A4</b>. El resultado de A4 alimenta el estado físico del siguiente ciclo. '
        'El sandbox <b>What-if</b> permite probar alternativas sin modificar el historial principal.</div>',
        unsafe_allow_html=True
    )
    render_physical()
    c1,c2=st.columns(2)
    ...
    if not st.session_state.history:
        ...
        st.line_chart(df[["Presión","Predicción"]])
        st.line_chart(df[["Servicio","Viabilidad","Éxito"]])
        st.line_chart(df[["Desperdicio LEAN"]])
...
# ---------- Footer ----------
st.divider()
st.caption("Supply Chain Digital Twin Learning Lab · MVP Streamlit · Cadena de suministro láctea · A1–A4 + SCIS + LEAN")
