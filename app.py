...
if page == "Diseño y optimización":
    ...
    st.caption("Define el tamaño de la red. El modelo genera automáticamente j1…jN y k1…kN.")

    net1, net2 = st.columns(2)
    with net1:
        od.number_of_plants = int(st.number_input(
            "Número de plantas / fábricas",
            min_value=1,
            value=max(1, int(getattr(od, "number_of_plants", 3))),
            step=1,
    ...
    with net2:
        od.number_of_cds = int(st.number_input(
            ...
            min_value=1,
            value=max(1, int(getattr(od, "number_of_cds", 4))),
            step=1,
            key="number_of_cds"
        ))
    ...
    with st.expander("⚙️ Parámetros avanzados de red", expanded=False):
        od.capacity_per_cd = st.number_input(
            "Capacidad de referencia por CD",
            min_value=1000., max_value=300000.,
            value=float(od.capacity_per_cd), step=1000.
        )
        st.caption(
            "Los nodos originales j1–j3 y k1–k4 conservan los datos base del GAMS. "
            "Los nodos adicionales se generan con perfiles determinísticos de referencia para permitir redes de dimensión variable."
        )
